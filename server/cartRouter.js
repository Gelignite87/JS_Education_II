...
router.post('/', (req, res) => {
    fs.readFile('./server/db/userCart.json', 'utf-8', (err, data) => {
        if (err) {
            ...
            fs.writeFile('./server/db/userCart.json', add(userCart, req), (err) => {
                if (err) {
                    res.send('{"result": 0}');
                } else {
                    res.send('{"result": 1}');
...
router.put('/:id', (req, res) => {
    fs.readFile('./server/db/userCart.json', 'utf-8', (err, data) => {
        if (err) {
            ...
            fs.writeFile('./server/db/userCart.json', change(userCart, req), (err) => {
                if (err) {
                    res.send('{"result": 0}');
                } else {
...
router.delete('/:id', (req, res) => {
    fs.readFile('./server/db/userCart.json', 'utf-8', (err, data) => {
        if (err) {
            ...
            fs.writeFile('./server/db/userCart.json', del(userCart, req), (err) => {
                if (err) {
                    res.send('{"result": 0}');
                } else {
                    res.send('{"result": 1}');
