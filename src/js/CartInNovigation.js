...
export const cartInNavigation = {
    ...
    methods: {
        addProduct(product) {
            ...
            if (find) {
                this.$root.putJson(`/api/cart/${find.id_product}?x=10&y=12&z=14`, { quantity: 1 })
                    .then((data) => {
                        console.log(data);
                        if (data.result === 1)
        ...
        remove(item) {
            this.$root.getJson('/api/cart')
                .then(data => {
                    if (data) {
                        if (item.quantity > 1) {
                            this.$root.putJson(`/api/cart/${item.id_product}?x=10&y=12&z=14`, { quantity: -1 })
                                .then((data) => {
                                    console.log(data);
                                    if (data.result === 1)
    ...
    mounted() {
        ...
        if (!withinBoundaries) {this.$root.state.cartInNavigation = false}})
    },
    template: `
